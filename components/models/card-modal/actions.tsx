...
interface ActinsProps {
  data: CardWithList;
}

export const Actions = ({ data }: ActinsProps) => {
  ...
  const onCopy = () => {
    executeCopyCard({id:JSON.stringify(data.id),boardId});
  }

  const onDelete = () => {
    executeDeleteCard({id:JSON.stringify(data.id),boardId});
  }
  return (
